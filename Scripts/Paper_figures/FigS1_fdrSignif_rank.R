...
#' wb:
#'   log:
#'    - snakemake: '`sm config["log_dir"] + "/snakemake/paper_figures/figS1_fdrSignif_rank.Rds"`'
#'   threads: 30
#'   resources:
#'     - mem_mb: 200000
#'   input:
#'     - variant_enrich_comparison_all: '`sm expand(config["DATADIR"] + "/GTEx_v8/{dataset}/plot_rds/FRASER2_enrichment/FRASER2_vs_competitors_fdrSignif_rv_recall_data_rare{snptype}.Rds", snptype=["Splicing", "MMSplice", "SpliceAI"], dataset=config["tissues_for_detailed_analysis"])`'
#'   output:
#'    - outPng: '`sm config["PAPER_FIGDIR"] + "/FigS1_fdrSignif_rank.png"`'
#'    - outPdf: '`sm config["PAPER_FIGDIR"] + "/FigS1_fdrSignif_rank.pdf"`'
#'   type: script
#'---
...
dt[, snptype:=factor(snptype, levels=c("rare MMSplice",
                                       "rare SpliceAI",
...
g_pr_fdrSignif_sup <- gg +
    # facet_grid(factor(snptype)~dataset) +
    facet_grid(dataset ~ factor(snptype), scales="free_y") +
    labs(x="Top N outliers", y="Recall of rare splice altering variants") +
    grids(color="white") +
    xlim(0, maxRank) +
    ...
    xlim(0, maxRank)
# ylim(0, rrdt[rank < maxRank, max(recall)]) +
# scale_x_log10(breaks=c(1e1, 1e3, 1e5), labels=scientific_10(c(1e1, 1e3, 1e5))) #+
