...
#' wb:
#'   log:
#'    - snakemake: '`sm config["log_dir"] + "/snakemake/{dataset_group}/fraser2_improvements/minK{k}_{q}_minN{n}/{dataset}/qqPlot_comparison_{implementation}.Rds"`'
#'   threads: 3
#'   resources:
#'     - mem_mb: 30000
#'   input:
#'     - fraser1_qq_data: '`sm config["DATADIR"] + "/{dataset_group}/fraser2_improvements/minK20_95_minN1/optQ/PCA/{dataset}/fraser1_qqPlot_data.tsv.gz"`'
#'     - fraser2_qq_data: '`sm config["DATADIR"] + "/{dataset_group}/fraser2_improvements/minK{k}_{q}_minN{n}/optQ/{implementation}/{dataset}/fraser2_qqPlot_data.tsv.gz"`'
#'   output:
#'     - wBhtml: '`sm config["htmlOutputPath"] + "/{dataset_group}/fraser2_improvements/minK{k}_{q}_minN{n}/optQ/{implementation}/{dataset}/global_qqPlots.html"`'
#'     - out_rds: '`sm config["DATADIR"] + "/{dataset_group}/fraser2_improvements/minK{k}_{q}_minN{n}/optQ/{implementation}/{dataset}/global_qqPlots.Rds"`'
#'     - outPng: '`sm config["htmlOutputPath"] + "/{dataset_group}/fraser2_improvements/minK{k}_{q}_minN{n}/optQ/{implementation}/{dataset}/global_qqPlots.png"`'
#'   type: noindex
#' output:
#'   html_document
#'---
...
ggplotLabelPsi <- function (type){
    ...
    vapply(type, FUN = function(x) switch(x, 
                jaccard = c(bquote(Intron ~ Jaccard ~ Index)), # c(bquote(atop(Intron ~ Jaccard, Index))), 
                psi5 = c(bquote(psi[5])), 
                psi3 = c(bquote(psi[3])), 
...
ggsave(plot=g, filename=snakemake@output$outPng, width=6, height=4)
