...
#' wb:
#'   log:
#'    - snakemake: '`sm config["log_dir"] + "/snakemake/GTEx_v8/reproducibility_fraser1paper/{snptype}__{deltaPsi}_reproducibility.Rds"`'     
#'   threads: 9
#'   resources:
#'    - mem_mb: 350000
#'   input:
#'     - variants:      '`sm config["DATADIR"] + "/GTEx_v8/variant_extraction/{snptype}_filtered_VariantsTable.tsv.gz"`'
#'     - annotation:    '`sm config["gtex_sample_anno"]`'
#'     - outlierCalls:  '`sm expand(config["DATADIR"] + "/GTEx_v8/reproducibility_fraser1paper/{dataset}__outlierStatus__{{deltaPsi}}.tsv.gz", dataset=config["tissues_for_reproducibility"])`'
#'   output:
#'     - plots:         '`sm config["DATADIR"] + "/GTEx_v8/reproducibility_fraser1paper/{snptype}__{deltaPsi}_reproducibility.RDS"`'
#'     - allres:        '`sm config["DATADIR"] + "/GTEx_v8/reproducibility_fraser1paper/{snptype}__{deltaPsi}_reproducibility.tsv.gz"`'
#'     - wBhtml: '`sm config["htmlOutputPath"] + "/GTEx_v8/reproducibility_fraser1paper/{snptype}__{deltaPsi}_reproducibility.html"`'
#'   type: noindex
#' output:
#'   html_document:
#'     code_folding: show
...
minTissuesTested  <- snakemake@config$required_tissues_per_gene
goodMethods       <- c("LeafcutterMD_p", "SPOT_p", "FRASER_p", "FRASER2_p")
...
annoTable <- table(anno[, .(INDIVIDUAL_ID, TISSUE)])
...
goodTissues <- names(which(colSums(annoTable) >= 100)) # 20
annoTable <- annoTable[,goodTissues]

plot(ecdf(rowSums(annoTable)), main="ECDF of number of tissues per subject")
grid()
goodSamples <- names(which(rowSums(annoTable) >= snakemake@config$required_tissues_per_individual)) # 20
length(goodSamples)
