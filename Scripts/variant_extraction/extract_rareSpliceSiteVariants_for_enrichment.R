...
#' wb:
#'   log:
#'    - snakemake: '`sm config["log_dir"] + "/variant_extraction/extract_rareSpliceSiteVars.Rds"`'   
#'   threads: 1
#'   resources:
#'     - mem_mb: 15000
#'   input:
#'     - variantTable: '`sm expand(config["DATADIR"] + "/{dataset_vcf_group}/variant_extraction/rareSpliceSiteVariantsTable.tsv", dataset_vcf_group=config["vcf_files"].keys())`' 
#'   output:
#'     - variantTable: '`sm expand(config["DATADIR"] + "/{dataset_vcf_group}/variant_extraction/rareSpliceSite_filtered_VariantsTable.tsv.gz", dataset_vcf_group=config["vcf_files"].keys())`'
#' output:
#'   html_document:
#'     code_folding: show
#'     code_download: TRUE
