#'---
#' title: Extract rare SpliceAI variants for enrichment
#' author: Christian Mertes, Ines Scheller
#' wb:
#'   log:
#'    - snakemake: '`sm config["log_dir"] + "/variant_extraction/extract_rareSpliceAIVars.Rds"`'   
#'   threads: 1
#'   resources:
#'     - mem_mb: 15000
#'   input:
#'     - variantTable_dir:  '`sm config["general_data_dir"] + "/absplice/data/results/gtex_v8_with_general_workflow/splicing_predictions/pred_variant_level/spliceai"`'
#'   output:
#'     - variantTable: '`sm expand(config["DATADIR"] + "/{dataset_vcf_group}/variant_extraction/rareSpliceAI_filtered_VariantsTable.tsv.gz", dataset_vcf_group=config["vcf_files"].keys(), allow_missing=True)`'
#' output:
#'   html_document:
#'     code_folding: show
#'     code_download: TRUE
...
spliceAIDir <- snakemake@input$variantTable_dir
spliceAI_cutoff <- 0.5 # as.numeric(snakemake@wildcards$cutoff) / 10 # wildcard=5 -> 0.5
...
#' Read in vcf file
#+ read vcf
variants <- rbindlist(lapply(list.files(spliceAIDir, recursive=TRUE), function(file){
    return(fread(file.path(spliceAIDir, file)))
}))

# apply cutoff on predicted delta_logit_psi
variants <- variants[delta_score >= spliceAI_cutoff,]

# cleanup table
variants <- variants[,.(variantID=variant, subjectID=sample, MAF=0.001,
                        delta_score, Gene=gene_id, SYMBOL=gene_name,
                        simple_conseq="SpliceAI", IMPACT="HIGH")]
...
#' 
hist(variants[,delta_score], breaks=100)

#' Keep only the gene level and most severe variant annotation
#' (remove transcript level)
variants <- variants[order(subjectID, Gene, -delta_score)]
