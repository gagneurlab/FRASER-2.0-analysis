...
#' wb:
#'   log:
#'    - snakemake: '`sm config["log_dir"] + "/snakemake/{dataset_group}/{dataset}/FRASER_vs_jaccard_rv_recall_{snptype}.Rds"`'
#'   py:
#'   - |
...
#'   output:
#'     - wBhtml: '`sm config["htmlOutputPath"] + "/{dataset_group}/variant_enrichment/{dataset}/FRASER_vs_jaccard_rv_recall_{snptype}.html"`'
#'     - gg_rds: '`sm config["DATADIR"] + "/{dataset_group}/{dataset}/plot_rds/FRASER2_enrichment/FRASER_vs_jaccard_rv_recall_plots_{snptype}.Rds"`'
#'     - rds: '`sm config["DATADIR"] + "/{dataset_group}/{dataset}/plot_rds/FRASER2_enrichment/FRASER_vs_jaccard_rv_recall_data_{snptype}.Rds"`'
#'     - recall_at_x_outliers_rds: '`sm config["DATADIR"] + "/{dataset_group}/{dataset}/plot_rds/FRASER2_enrichment/FRASER_vs_jaccard_recall_at_x_outliers_{snptype}.Rds"`'
#'   type: noindex
#' output:
#'   html_document
#'---
...
getMetricLabels <- function(methods){
    vapply(methods, FUN = function(x) switch(x, 
                        ...
                        FRASER2=c(bquote(FRASER2)),
                        IntronJaccardIndex = c(bquote(Intron~Jaccard~Index)),
                        SPOT = c(bquote(SPOT)),
                        LeafcutterMD = c(bquote(LeafcutterMD))), 
