...
#' wb:
#'   log:
#'    - snakemake: '`sm config["log_dir"] + "/snakemake/{dataset_group}/{dataset}/FRASER2_filtering_rv_recall_pc{pc}_rho{rho}_delta{delta}_{snptype}.Rds"`'
#'   py:
#'   - |
...
#'     - mem_mb: 50000
#'   input:
#'     - k20_q95_n1_p: '`sm config["DATADIR"] + "/{dataset_group}/{dataset}/minK20_95_minN1/PCA__pc{pc}/optQ__newFilt/jaccard/pval_matrix__{delta}__5__{rho}__FALSE.tsv.gz"`'
#'     - k20_q95_n5_p: '`sm config["DATADIR"] + "/{dataset_group}/{dataset}/minK20_95_minN5/PCA__pc{pc}/optQ__newFilt/jaccard/pval_matrix__{delta}__5__{rho}__FALSE.tsv.gz"`'
#'     - k20_q95_n10_p: '`sm config["DATADIR"] + "/{dataset_group}/{dataset}/minK20_95_minN10/PCA__pc{pc}/optQ__newFilt/jaccard/pval_matrix__{delta}__5__{rho}__FALSE.tsv.gz"`'
#'     - k20_q75_n1_p: '`sm config["DATADIR"] + "/{dataset_group}/{dataset}/minK20_75_minN1/PCA__pc{pc}/optQ__newFilt/jaccard/pval_matrix__{delta}__5__{rho}__FALSE.tsv.gz"`'
#'     - k20_q75_n5_p: '`sm config["DATADIR"] + "/{dataset_group}/{dataset}/minK20_75_minN5/PCA__pc{pc}/optQ__newFilt/jaccard/pval_matrix__{delta}__5__{rho}__FALSE.tsv.gz"`'
#'     - k20_q75_n10_p: '`sm config["DATADIR"] + "/{dataset_group}/{dataset}/minK20_75_minN10/PCA__pc{pc}/optQ__newFilt/jaccard/pval_matrix__{delta}__5__{rho}__FALSE.tsv.gz"`'
#'     - k20_q25_n1_p: '`sm config["DATADIR"] + "/{dataset_group}/{dataset}/minK20_25_minN1/PCA__pc{pc}/optQ__newFilt/jaccard/pval_matrix__{delta}__5__{rho}__FALSE.tsv.gz"`'
#'     - k20_q25_n5_p: '`sm config["DATADIR"] + "/{dataset_group}/{dataset}/minK20_25_minN5/PCA__pc{pc}/optQ__newFilt/jaccard/pval_matrix__{delta}__5__{rho}__FALSE.tsv.gz"`'
#'     - k20_q25_n10_p: '`sm config["DATADIR"] + "/{dataset_group}/{dataset}/minK20_25_minN10/PCA__pc{pc}/optQ__newFilt/jaccard/pval_matrix__{delta}__5__{rho}__FALSE.tsv.gz"`'
#'     - k20_q5_n10_p: '`sm config["DATADIR"] + "/{dataset_group}/{dataset}/minK20_5_minN10/PCA__pc{pc}/optQ__newFilt/jaccard/pval_matrix__{delta}__5__{rho}__FALSE.tsv.gz"`'
#'     - variant_table: '`sm get_input_variants`'
#'   output:
#'     - wBhtml: '`sm config["htmlOutputPath"] + "/{dataset_group}/variant_enrichment/{dataset}/FRASER2_filtering_rv_recall_pc{pc}_rho{rho}_delta{delta}_{snptype}.html"`'
#'     - gg_rds: '`sm config["DATADIR"] + "/{dataset_group}/{dataset}/plot_rds/FRASER2_enrichment/FRASER2_filtering_rv_recall_plots_pc{pc}_rho{rho}_delta{delta}_{snptype}.Rds"`'
#'     - rds: '`sm config["DATADIR"] + "/{dataset_group}/{dataset}/plot_rds/FRASER2_enrichment/FRASER2_filtering_rv_recall_data_pc{pc}_rho{rho}_delta{delta}_{snptype}.Rds"`'
#'     - recall_at_x_outliers_rds: '`sm config["DATADIR"] + "/{dataset_group}/{dataset}/plot_rds/FRASER2_enrichment/FRASER2_filtering_recall_at_x_outliers_pc{pc}_rho{rho}_delta{delta}_{snptype}.Rds"`'
#'   type: noindex
#' output:
#'   html_document
#'---
...
res_for_comparision <- lapply(names(input_files), function(method_name){
    ...
    # dt[, ngenes_detected:=nrow(pval_mat)]
...
#'
#' ## Get tested genes for all methods 
#' 
...
ngenes_detected <- lapply(names(input_files), function(method_name){ nrow(fread(input_files[[method_name]]))})
names(ngenes_detected) <- names(input_files)
rank_cutoff_20 <- n_samples*20
recall_20_dt <- rrdt[rank == rank_cutoff_20, ]
recall_20_dt[, k:=as.numeric(sapply(regmatches(Method, regexec(pattern="k([0-9]+)", text=Method)), "[", 2))]
recall_20_dt[, q:=as.numeric(sapply(regmatches(Method, regexec(pattern="q([0-9]+)", text=Method)), "[", 2))]
recall_20_dt[, n:=as.numeric(sapply(regmatches(Method, regexec(pattern="n([0-9]+)", text=Method)), "[", 2))]
recall_20_dt[, pc:=as.numeric(snakemake@wildcards$pc)]
recall_20_dt[, rho:=as.numeric(snakemake@wildcards$rho)]
recall_20_dt[, delta:=as.numeric(snakemake@wildcards$delta)]
recall_20_dt[, ngenes_detected := ngenes_detected[Method] ]
ggplots[['recall_at_20_outliers']] <- ggplot(recall_20_dt, aes(factor(n), recall)) +
    facet_wrap(~ k + q) +
    geom_bar(stat="identity") + 
    labs(x="N (q% of samples have N >= ...)")
ggplots[['recall_at_20_outliers']]


rank_cutoff_10 <- n_samples*10
recall_10_dt <- rrdt[rank == rank_cutoff_10, ]
recall_10_dt[, k:=as.numeric(sapply(regmatches(Method, regexec(pattern="k([0-9]+)", text=Method)), "[", 2))]
recall_10_dt[, q:=as.numeric(sapply(regmatches(Method, regexec(pattern="q([0-9]+)", text=Method)), "[", 2))]
recall_10_dt[, n:=as.numeric(sapply(regmatches(Method, regexec(pattern="n([0-9]+)", text=Method)), "[", 2))]
recall_10_dt[, pc:=as.numeric(snakemake@wildcards$pc)]
recall_10_dt[, rho:=as.numeric(snakemake@wildcards$rho)]
recall_10_dt[, delta:=as.numeric(snakemake@wildcards$delta)]
recall_20_dt[, ngenes_detected := ngenes_detected[Method] ]
ggplots[['recall_at_10_outliers']] <- ggplot(recall_10_dt, aes(factor(n), recall)) +
    facet_wrap(~ k + q) +
    geom_bar(stat="identity") + 
    labs(x="N (q% of samples have N >= ...)")
