#'---
#' title: Compare FRASER2 (jaccard) to FRASER1 (only metric changes, everything else as in FRASER1)
#' author: Ines Scheller
#' wb:
#'   log:
#'    - snakemake: '`sm config["log_dir"] + "/snakemake/{dataset_group}/{dataset}/FRASER_vs_FRASER2_venn_rv_recall_{snptype}.Rds"`'
#'   py:
#'   - |
...
#'     - FRASER2_p: '`sm config["DATADIR"] + "/{dataset_group}/{dataset}/minK20_25_minN10/PCA__pc0.1/optQ__newFilt/jaccard/pval_matrix__0.1__5__1__FALSE.tsv.gz"`'
#'     - variant_table: '`sm get_input_variants`'
#'   output:
#'     - wBhtml: '`sm config["htmlOutputPath"] + "/{dataset_group}/variant_enrichment/{dataset}/FRASER_vs_FRASER2_venn_rv_recall_{snptype}.html"`'
#'     - gg_rds: '`sm config["DATADIR"] + "/{dataset_group}/{dataset}/plot_rds/FRASER2_enrichment/FRASER_vs_FRASER2_venn_rv_recall_plots_{snptype}.Rds"`'
#'     - rds: '`sm config["DATADIR"] + "/{dataset_group}/{dataset}/plot_rds/FRASER2_enrichment/FRASER_vs_FRASER2_venn_rv_recall_data_{snptype}.Rds"`'
#'     - recall_at_x_outliers_rds: '`sm config["DATADIR"] + "/{dataset_group}/{dataset}/plot_rds/FRASER2_enrichment/FRASER_vs_FRASER2_venn_recall_at_x_outliers_{snptype}.Rds"`'
#'   type: noindex
#' output:
#'   html_document
#'---
...
pval_for_venn <- 1e-5
# FRASER only
res_all[, FRASER_only_p := FRASER_p]
res_all[FRASER2_p <= pval_for_venn, FRASER_only_p := NA]
# FRASER2 only
res_all[, FRASER2_only_p := FRASER2_p]
res_all[FRASER_p <= pval_for_venn, FRASER2_only_p := NA]
# intersection
res_all[, intersection_p := rowMeans(cbind(FRASER2_p, FRASER_p))]
res_all[FRASER2_p > pval_for_venn | FRASER2_p > pval_for_venn, intersection_p := NA]
...
rrdt[Method == "FRASER_only", Method := "FRASER & not FRASER2"]
rrdt[Method == "FRASER2_only", Method := "FRASER2 & not FRASER"]
rrdt[Method == "intersection", Method := "FRASER & FRASER2"]
method_order <- c("FRASER", "FRASER2", "FRASER & not FRASER2", "FRASER2 & not FRASER", "FRASER & FRASER2")
...
dt_tmp[Method == "FRASER_only", Method := "FRASER & not FRASER2"]
dt_tmp[Method == "FRASER2_only", Method := "FRASER2 & not FRASER"]
dt_tmp[Method == "intersection", Method := "FRASER & FRASER2"]
...
# stop intersection line after cutoff
rrdt <- rrdt[Method != "FRASER & FRASER2" | rank <= dt4cutoffs[Method == "FRASER & FRASER2" & Cutoff == "10^-5", rank],]
...
for(maxRank in c(10000, 20000, 50000, 100000)){
    ...
    ggplots[[paste0('recall_n=', maxRank)]] <- plotRecallRankForEnrichment(
        ...
        scale_color_manual(values=c("dodgerblue3", "purple4", "cornflowerblue", "purple1", "darkgreen"),
    ...
    ggplots[[paste0('recall_n=', maxRank, "_xlog")]] <- plotRecallRankForEnrichment(
        ...
        geom_abline(intercept=0, slope=rrdt[,1/max(rank)], col="firebrick", linetype="dashed") + 
        # scale_color_brewer(palette="Paired", labels=metric_labels) + 
        scale_color_manual(values=c("orange", "darkolivegreen", "lightblue", "purple4"),
                           labels=metric_labels) +
        scale_shape_discrete(labels=function(x)parse(text=x)) +
        guides(linetype = "none") + 
        guides(shape=guide_legend(title=ifelse(all(dt4cutoffs[,Type == "P-value"]), "Nominal\np-value cutoff", "Cutoff"), order = 2),
