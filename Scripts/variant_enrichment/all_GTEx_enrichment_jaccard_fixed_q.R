#'---
#' title: Compare jaccard to old version, on fixed q, for all tissues
...
#'     - mem_mb: 25000
#'   input:
#'     - rareSplice: '`sm expand(config["DATADIR"] + "/{dataset}/plot_rds/q{q}/jaccard_enrichment_data.Rds", dataset=config["tissues_for_reproducibility"], allow_missing=True)`'
#'   output:
#'    - wBhtml: '`sm config["htmlOutputPath"] + "/variant_enrichment/GTEx_tissues/q{q}_enrichment_jaccard.html"`'
#'    - outPng: '`sm config["htmlOutputPath"] + "/variant_enrichment/GTEx_tissues/q{q}_enrichment_jaccard.png"`'
#'   type: noindex
#' output:
#'   html_document
#'---
...
METHODS_2_PLOT <- c("FRASER_p", "FRASER2_delta>=0.1", "FRASER2_delta>=0.2", "FRASER2_delta>=0.3", "FRASER2_delta>=0.4", "FRASER2_delta>=0.5")
